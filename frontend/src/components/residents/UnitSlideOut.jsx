    ...
    <>
      ...
      <div ref={dialogRef} {...dialogProps} className="fixed inset-y-0 right-0 w-full max-w-sm bg-white shadow-2xl z-50 transform transition-transform duration-300 flex flex-col border-l border-slate-200">
        ...
        {unit.is_occupied && (
          ...
          {/* The only password recovery path a resident has. */}
          <button
            onClick={() => {
              onClose();
              onReissuePassword(unit);
            }}
            className="w-full px-3 py-2.5 bg-white border border-slate-200 text-slate-600 text-xs font-bold rounded-xl hover:bg-slate-100 transition-colors shadow-xs flex items-center justify-center gap-1.5"
          >
            <KeyRound className="w-3.5 h-3.5" /> Re-issue password
          </button>
          </div>
        )}
      </div>
